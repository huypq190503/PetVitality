<nav class="navbar-default navbar-side" role="navigation">
            <div class="sidebar-collapse">
                <ul class="nav" id="main-menu">
                    <li>
                        <a href="?act=dashboard"><i class="fa fa-dashboard"></i> Dashboard</a>
                    </li>
                    <li>
                        <a href="?act=listdm"><i class="fa fa-bar-chart-o"></i> Quản lý danh mục</a>
                    </li>
                    <li>
                        <a href="?act=listsp"><i class="fa fa-desktop"></i> Quản lý sản phẩm</a>
                    </li>
                    <li>
                        <a href="?act=list_account"><i class="fa fa-qrcode"></i> Quản lý tài khoản</a>
                    </li>
                    
                    <li>
                        <a href="?act=dsbl"><i class="fa fa-table"></i> Quản lý bình luận</a>
                    </li>
                    <li>
                        <a href="?act=dsdh"><i class="fa fa-edit"></i> Quản lý đơn hàng</a>
                    </li>
                    <li>
                        <a href="#"><i class="fa fa-bar-chart-o"></i> Thống kê</a>
                    </li>
                    <li>
                        <a href="../index.php"><i class="fa fa-desktop"></i> Quay lại trang chủ</a>
                    </li>
                </ul>

            </div>

        </nav>